...
m_snakeDirection({16.f,0.f}),m_elapsedTime(sf::Time::Zero),m_score(0),m_isPause(false)
{
    srand(time(nullptr));
}
...
void GamePlay::Init(){
    m_context->m_assets->AddTexture(AssetID::GRASS,"assets/textures/grass.png",true);
    m_context->m_assets->AddTexture(AssetID::FOOD,"assets/textures/food.png");
    m_context->m_assets->AddTexture(AssetID::WALL,"assets/textures/wall.png",true);
    m_context->m_assets->AddTexture(AssetID::SNAKE,"assets/textures/snake.png");

    m_grass.setTexture(m_context->m_assets->GetTexture(AssetID::GRASS));
    m_grass.setTextureRect(m_context->m_window->getViewport(m_context->m_window->getDefaultView()));
    for(auto &wall : m_walls){
        wall.setTexture(m_context->m_assets->GetTexture(AssetID::WALL));
    }
    ...
    m_food.setTexture(m_context->m_assets->GetTexture(AssetID::FOOD));
    m_food.setPosition(m_context->m_window->getSize().x / 2, m_context->m_window->getSize().y / 2);

    m_snake.Init(m_context->m_assets->GetTexture(AssetID::SNAKE));

    m_scoreText.setFont(m_context->m_assets->GetFont(AssetID::MAIN_FONT));
